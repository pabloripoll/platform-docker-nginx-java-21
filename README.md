...
# INFRASTRUCTURE PLATFORM FOR JAVA 21
...
- **REST API Platform:** Linux Alpine version 3.23 + NGINX version 1.28 *(or latest by APK)* + Java 21 + Maven
...
- [NGINX + JAVA 21](./platform/nginx-java-21/README.md)
<br><br>
...
- By configuring the REST API container with e.g. `APIREST_CAAS_MEM=256M` *(CAAS = Container As A Service)*, remember to set the same RAM value into container local configuration files that will be mounted into the container.
<br>
...
# API - LOCAL
APIREST_PLTF=nginx-java-21                              # <- platform assets directory's name ---------------------------------------------------------------> #
APIREST_IMGK=alpine3.23-nginx-28-java-21                # <- real main image keys to manage automations for sharing resources -------------------------------> #
APIREST_PORT=7714                                       # <- local machine port opened for container service ------------------------------------------------> #
...
APIREST_CAAS_MEM=256M                                   # <- container's maximum RAM usage to apply by docker-compose ---------------------------------------> #
